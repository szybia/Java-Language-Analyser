...
private JLabel mylab;													//Main title
	...
	public FancyWordsEdit(String title, ImageIcon img)
	{
		super(title);
		setSize(850, 500);
	...
	public void actionPerformed (ActionEvent e)
	{
	    ...
	    else if (e.getSource().equals(addWord))
	    {
	    	...
	    	if(input != null && !input.isEmpty())
	    	{
	    		try
	    		{
	    			addWordToFile = new PrintWriter(new FileWriter("Resources\\fancyWords.txt", true));
	    ...
	    else if (e.getSource().equals(removeWord))
	    {
	    	...
	    	if(input != null && !input.isEmpty())
	    	{
	    		try
	    		{
	    			org_file = new File("Resources\\fancyWords.txt");
	    			newScanner = new Scanner(org_file);
	    			temp_file = File.createTempFile("fancyWords", ".txt",
	    ...
	    else if (e.getSource().equals(resetList))
	    {
	    	try
    		{
    			org_file = new File("Resources\\fancyBackup.txt");
    			newScanner = new Scanner(org_file);
    			temp_file = new File("Resources\\fancyWords.txt");
	...
	{
		mylab = new JLabel("Sophisticated Word Dictionary", SwingConstants.CENTER);
	...
	public void initFileReader()
	{
		try
		{
			fileRead = new FileReader("resources\\fancyWords.txt");
		}
	...
	public void initTextField()
	{
		...
		try
		{
			wordArea.read(fileRead, "Resources\\fancyWords.txt");
			fileRead.close();
		}
		catch (IOException e)
		{
